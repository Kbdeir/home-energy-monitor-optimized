...
{
  double sumxi = 0;
  int8_t numsamples = 75; // 75 is optimal
  ...
  for (unsigned int n = 0; n < Number_of_Samples; n++)
  {
    ...
    //float adjustedInputValue = analogReadAdjusted(inPinI);
    //sampleI = adjustedInputValue;
    /*
    int val = analogRead(inPinI);
    double x = val;
    double y = -3e-12*pow(x, 3) - 7e-10*pow(x, 2) + 0.0003*x + 0.0169;
    sampleI = std::min(255, std::max(0, int(round(y*UINT8_MAX))));
    */
